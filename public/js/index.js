...
async function AddSeller(e) {
  ...
  console.log(obj);
  const res = await axios.post("http://localhost:4000/add-seller", obj);
  showItem(res.data.newSeller);
}

window.addEventListener("DOMContentLoaded", async () => {
  const res = await axios.get("http://localhost:4000/all-seller");
...
function showItem(obj) {
  ...
  newItem.appendChild(
    document.createTextNode(obj.name + " " + obj.price + " " + obj.type)
  );
  ...
  deleteBtn.onclick = async (e) => {
    ...
    await axios.delete("http://localhost:4000/delete-seller/" + id);
  };
